...
async function executeDelete(client, stack, confirmed) {
  if (toSkip(stack.StackName)) {
    console.log();
    console.log(`Skipping stack ${stack.StackName}...`);
    return;
  }
...
function toSkip(stackName) {
  return TOSKIP.some((skippedStack) => {
    const regex = new RegExp(skippedStack);
