...
async function getConfiguredAwsClient(promptRegion) {
  process.env.AWS_SDK_LOAD_CONFIG = true;
  const configInfo = readConfigInfo() || {};
  if (configInfo.configLevel === constants.Tag) {
    if (configInfo.useProfile) {
      ...
      aws.config.loadFromPath(pathManager.getConfigFilePath());
    }
  }
  ...
  if (promptRegion) {
    const regionPrompt = {
      ...
      default: constants.DefaultAWSRegion,
    };
    const answer = await inquirer.prompt(regionPrompt);
    aws.config.update({ region: answer.region });
  }
...
async function configure() {
  const dotAmplicleanDirPath = pathManager.getDotDirPath();
  fs.ensureDirSync(dotAmplicleanDirPath);
...
function readConfigInfo() {
  ...
  if (fs.existsSync(configInfoFilePath)) {
    configInfo = JSON.parse(fs.readFileSync(configInfoFilePath));
  }
  return configInfo;
}
